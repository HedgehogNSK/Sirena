...
public class UnsubscribeCommand : BotCustomCommmand
{
  private IMongoCollection<UserRepresentation> usersCollection;
  private IMongoCollection<SirenRepresentation> sirenCollection;
  public UnsubscribeCommand(string name, string description, IMongoDatabase db)
  ...
  public async override void Execute(Message message)
  {
    ...
    if(string.IsNullOrEmpty(param) || !ObjectId.TryParse(param, out ObjectId id)){
      ...
      Program.messageSender.Send(message.Chat.Id, failMessageText);
      return;
    }
