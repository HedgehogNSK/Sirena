...
public class CallSirenaCommand : BotCustomCommmand
{
  ...
  const string errorNoSubscribers = "The sirena: {0} doesn't have any subsribers";
  ...
  const string notificationBase =  "ATTENTION! {0} called by {1} at {2}\n";
  ...
  public async override void Execute(Message message)
  {
    ...
    if (sirena.LastCall + delay > now )
    {

      responseText = string.Format(errorRecentlyCall, sirena, sirena.LastCall.ToString(dateFormat));
      Program.messageSender.Send(message.Chat.Id, responseText);
      return;
    }

    var username = BotTools.GetUsername(message.From);
    string notification = string.Format(notificationBase, sirena.Title, username, dateString);
    ...
    responseText = string.Format(successMessage, sirena.Listener.Length);
    Program.messageSender.Send(message.Chat.Id, responseText);

    var result = requests.SetCallDate(sirena.Id, now);

    foreach (var listener in sirena.Listener)
